...
void image::scaleRender(HDC hdc, int destX, int destY, float scaleW, float scaleH)
{
	//스트레치이미지 처음 사용하냐?
	//이미지 스케일링을 사용할 수 있도록 초기화 해라
	if (!_scaleImage) this->initForStretchBlt();

	_scaleImage->width = _imageInfo->width * scaleW;
	_scaleImage->height = _imageInfo->height * scaleH;
